...
void writeBMP(const char *filename, const uint8_t image[HEIGHT][WIDTH]) {
    ...
    for (int i = 0; i < HEIGHT; i++) {
        for (int j = 0; j < WIDTH; j++) {
            uint8_t quantized_value = ((image[i][j] / 16) * 16) + 8; // Adjusting to mid-point of each bin
...
int main() {
    ...
    writeBMP("../Input/QuantizedImage16.bmp", image);
    printf("Quantized image saved as QuantizedImage16.bmp\n");
    return 0;
}
